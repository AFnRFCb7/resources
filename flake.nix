                                                ...
                                                let
                                                                ...
                                                                text =
                                                                    ''
                                                                        ...
                                                                        then
                                                                            ...
                                                                            then
                                                                                echo "${ label } command succeeded but it generated standard-error"
                                                                                exit 133
                                                                            fi
                                                                            ${ if status != 0 then ''echo "${ label } command succeeded but we expected the status to be ${ builtins.toString status } and we observed 0" >&2'' else "# " }
                                                    ...
                                                    root =
                                                        writeShellApplication
                                                            {
                                                                name = "root" ;
                                                                runtimeInputs = [ coreutils invoke-resource ] ;
                                                                text =
                                                                    ''
                                                                        OUT="$1"
                                                                        touch "$OUT"
                                                        ...
                                                        scripts =
                                                            {
                                                                ...
                                                                sequential =
                                                                    ''
                                                                        ...
                                                                        exec 205> ${ resources-directory }/counter
                                                                        flock -x 205
                                                                        if [[ -s ${ resources-directory }/counter ]]
                                                                        then
                                                                            OLD="$( < ${ resources-directory }/random )" || exit ${ builtins.toString hidden-error }
                                                                        else
                                                                            OLD="00000000"
                                                                        fi
                                                                        NEW=$(( OLD + 1 ))
                                                                        echo "$NEW" > ${ resources-directory }/counter
                                                                        chmod 0600 ${ resources-directory }/counter
                                                                        printf "%08d\n" "$NEW"
                                                                    '' ;
                                                                setup =
                                                                    if builtins.typeOf init == "null" then
                                                                        ...
                                                                        ''
                                                                            ...
                                                                            then
                                                                                ...
                                                                                then
                                                                                    ${ init-application }/bin/init-application "${ builtins.concatStringsSep "" [ "$" "{" "ARGUMENTS[@]" "}" ] }" < "$STANDARD_INPUT_FILE" > "$STANDARD_OUTPUT" 2> "$STANDARD_ERROR"
                                                                                    STATUS="$?"
                                                                                    flock -u 202
                                                                                    exec 202>&-
                                                                                    rm --force "$STANDARD_INPUT_FILE"
                                                                                    echo "STANDARD_ERROR=$STANDARD_ERROR" >&2
                                                                                    if [[ "$STATUS" == 0 ]] && [[ -n "$STANDARD_ERROR" ]] && [[ "$( find "${ resources-directory }/mounts/$HASH" -mindepth 1 -maxdepth 1 -exec basename {} \; | LC_ALL=C sort | tr --delete "\n" | sha512sum | cut --bytes -128 )" == ${ builtins.hashString "sha512" ( builtins.concatStringsSep "" ( builtins.sort builtins.lessThan targets ) ) } ]]
                                                                                    then
                                                                                        nohup good "$HASH" "$ORIGINATOR_PID" "$TRANSIENT" "$STATUS" "$STANDARD_OUTPUT" "$STANDARD_ERROR" "$HAS_STANDARD_INPUT" "$STANDARD_INPUT" "${ builtins.concatStringsSep "" [ "$" "{" "ARGUMENTS[@]" "}" ] }" > /dev/null 2>&1 &
                                                                                        echo -n "${ resources-directory }/mounts/$HASH"
                                                                                    else
                                                                                        nohup bad "$HASH" "$TRANSIENT" "$STATUS" "$STANDARD_OUTPUT" "$STANDARD_ERROR" "$HAS_STANDARD_INPUT" "$STANDARD_INPUT" "${ builtins.concatStringsSep "" [ "$" "{" "ARGUMENTS[@]" "}" ] }" > /dev/null 2>&1 &
                                                                                        exit ${ builtins.toString initialization-error }
                                                                                    fi
                                                                                else
                                                                                    ${ init-application }/bin/init-application "${ builtins.concatStringsSep "" [ "$" "{" "ARGUMENTS[@]" "}" ] }" > "$STANDARD_OUTPUT" 2> "$STANDARD_ERROR"
                                                                                    STATUS="$?"
                                                                                    flock -u 202
                                                                                    exec 202>&-
                                                                                    if [[ "$STATUS" == 0 ]] && [[ -n "$STANDARD_ERROR" ]] && [[ "$( find "${ resources-directory }/mounts/$HASH" -mindepth 1 -maxdepth 1 -exec basename {} \; | LC_ALL=C sort | tr --delete "\n" | sha512sum | cut --bytes -128 )" == ${ builtins.hashString "sha512" ( builtins.concatStringsSep "" ( builtins.sort builtins.lessThan targets ) ) } ]]
